...
const dataURL="https://dev-cs55-13-sonam.pantheonsite.io/wp-json/twentytwentyone-child/v1/addresses";

// function returns ids for all json objects in array
export async function getAllIds3() {
    let jsonString;
    try {
  ...
  export async function getSortedAddressList() {
    let jsonString;
    try {
      // next line uses got synchronously to retrieve via https our json data from wp site
      jsonString = await got(dataURL);
  ...
  export async function getData3(idRequested) {
    let jsonString;
    try {
      // next line uses got synchronously to retrieve via https our json data from wp site
      jsonString = await got(dataURL);
